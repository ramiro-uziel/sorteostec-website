<footer class="">
  <div
    class="absolute w-full mt py-10 md:py-20 text-xs pointer-events-auto bg-st-blue text-white flex flex-col gap-4 md:gap-8 justify-center items-center"
  >
    <img
      src="src/assets/images/Principal-Blanco.png"
      alt="logotipo"
      class="w-[200px]"
    />
    <div class="flex flex-col gap-4 pl-1 items-center justify-center">
      <div class="flex flex-row gap-2 text-xl text-st-blue">
        <a
          href="#"
          class="bg-st-blue-light border-2 border-transparent hover:bg-st-blue hover:text-st-blue-light hover:border-st-blue-light rounded-full duration-100"
          ><i class="fa-brands fa-facebook p-2" /></a
        >
        <a
          href="#"
          class="bg-st-blue-light border-2 border-transparent hover:bg-st-blue hover:text-st-blue-light hover:border-st-blue-light rounded-full duration-100"
          ><i class="fa-brands fa-instagram p-2" /></a
        >
        <a
          href="#"
          class="bg-st-blue-light border-2 border-transparent hover:bg-st-blue hover:text-st-blue-light hover:border-st-blue-light rounded-full duration-100"
          ><i class="fa-brands fa-whatsapp p-2" /></a
        >
        <a
          href="#"
          class="bg-st-blue-light border-2 border-transparent hover:bg-st-blue hover:text-st-blue-light hover:border-st-blue-light rounded-full duration-100"
          ><i class="fa-brands fa-youtube p-2" /></a
        >
      </div>
      <div class="flex flex-row max-w-60">
        <img src="src/assets/icons/18.svg" alt="boleto" class="w-[30px]" />

        <p class=" text-center">
          Para participar en Sorteos Tec deberás ser <b class="pr-1"
            >mayor de 18 años</b
          >
        </p>
      </div>
      <div class="flex flex-row">
        <p class=" text-center pt-5">
          © 2024 Sorteos Tec. Todos los derechos reservados
        </p>
      </div>
    </div>
  </div>
</footer>
